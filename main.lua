...
function MapKey(key)
    ...
    if not menu or not menu.keyBindings then
        return key
    end
...
function love.load()
    ...
    bullets:init(world) -- 1. Inicjalizacja świata Windfielda dla pocisków
    ...
    local startingItems = {"BUZZGAREN", "StaminaRing", "helmet", "chestplate", "leggins", "boots", "Shotgun"}
    ...
    if debugMode then
        ...
        for _, item in ipairs(inventory.itemsOnMap) do
            if not item.collected then
                table.insert(uncollectedCoins, item)
            end
        end
        line("-----------------------------------")
