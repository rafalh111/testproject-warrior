...
-- Usunięto require, referencje zostaną przekazane w init
local enemies = nil
local player = nil
local walls = {} 
...
function bullets:load()
    self.list = list
    self.speed = 1500
    self.shotgunSpeed = 900
    self.radius = 5
end

function bullets:init(windfieldWorld, enemiesModule, playerModule)
...
function bullets:spawn(x, y, dx, dy, damage, isShotgun)
    ...
    local bullet = {
        x = x,
        y = y,
        velX = dx * currentSpeed,
        velY = dy * currentSpeed,
        life = 2,
